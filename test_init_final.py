...
class taskCog(commands.Cog): 
			...
			try:
				...
				if log_stream.getvalue().find("Awaiting") != -1:
					...
					for chnl in channel_list:
						await self.bot.get_channel(chnl).send( '< 디코접속에러! 잠깐 나갔다 올께요! >', tts=False)
					await dbSave()
					break
				...
				if channel != '':
					...
					if len(self.bot.voice_clients) == 0 and self.checker and basicSetting[21] == "1":
						...
						if not self.bot.voice_clients[0].is_connected():
							...
							for chnl in channel_list:
								await self.bot.get_channel(chnl).send( '< 음성 채널 접속에 실패하였습니다. 잠시 후 음성 채널 접속을 시도해주세요! >')
							self.checker = False
							pass
					...
					for i in range(fixed_bossNum):
						...
						if fixed_bossTime[i] <= priv0 and fixed_bossTime[i] > priv:
							if basicSetting[3] != '0':
								if fixed_bossFlag0[i] == False:
									fixed_bossFlag0[i] = True
									temp_msg = "```" + fixed_bossData[i][0] + ' ' + basicSetting[3] + '분 전 ' + fixed_bossData[i][3] +' [' +  fixed_bossTime[i].strftime('%H:%M:%S') + ']```'
									asyncio.ensure_future(self.reaction_check(temp_msg))
									try:
										if basicSetting[21] == "1":
						...
						if fixed_bossTime[i] <= priv and fixed_bossTime[i] > now and fixed_bossFlag0[i] == True :
							if basicSetting[1] != '0' :
								if fixed_bossFlag[i] == False:
									...
									temp_msg = "```" + fixed_bossData[i][0] + ' ' + basicSetting[1] + '분 전 ' + fixed_bossData[i][3] +' [' +  fixed_bossTime[i].strftime('%H:%M:%S') + ']```'
									asyncio.ensure_future(self.reaction_check(temp_msg))
									try:
										if basicSetting[21] == "1":
						...
						if fixed_bossTime[i] <= now and fixed_bossFlag[i] == True and fixed_bossFlag0[i] == True :
							...
							for chnl in channel_list:
								await self.bot.get_channel(chnl).send(embed=embed, tts=False)
							try:
								if basicSetting[21] == "1":
					...
					for i in range(bossNum):
						...
						if bossTime[i] <= priv0 and bossTime[i] > priv:
							if basicSetting[3] != '0':
								if bossFlag0[i] == False:
									bossFlag0[i] = True
									if bossData[i][6] != '' :
										temp_msg = "```" + bossData[i][0] + ' ' + basicSetting[3] + '분 전 ' + bossData[i][3] + " [" +  bossTimeString[i] + "]" + '\n<' + bossData[i][6] + '>```'
										asyncio.ensure_future(self.reaction_check(temp_msg))
									else :
										temp_msg = "```" + bossData[i][0] + ' ' + basicSetting[3] + '분 전 ' + bossData[i][3] + " [" +  bossTimeString[i] + "]```"
										asyncio.ensure_future(self.reaction_check(temp_msg))
						...
						if bossTime[i] <= priv and bossTime[i] > now and bossFlag0[i] == True:
							if basicSetting[1] != '0' :
								if bossFlag[i] == False:
									bossFlag[i] = True
									if bossData[i][6] != '' :
										temp_msg = "```" + bossData[i][0] + ' ' + basicSetting[1] + '분 전 ' + bossData[i][3] + " [" +  bossTimeString[i] + "]" + '\n<' + bossData[i][6] + '>```'
										asyncio.ensure_future(self.reaction_check(temp_msg))
									else :
										temp_msg = "```" + bossData[i][0] + ' ' + basicSetting[1] + '분 전 ' + bossData[i][3] + " [" +  bossTimeString[i] + "]```"
										asyncio.ensure_future(self.reaction_check(temp_msg))
						...
						if bossTime[i] <= now and bossFlag0[i] == True and bossFlag[i] == True :
							...
							for chnl in channel_list:
								await self.bot.get_channel(chnl).send(embed=embed, tts=False, delete_after=60)
							try:
								if basicSetting[21] == "1":
							...
							if (bossTime[i]+datetime.timedelta(days=-365)) <= aftr:
								if basicSetting[2] != '0' and basicSetting[22] != '0' and bossFlag[i] == True and bossFlag0[i] == True and bossMungFlag[i] == True :
									if int(basicSetting[17]) <= bossMungCnt[i] and int(basicSetting[17]) != 0:
										...
										if bossData[i][2] == '0':
											for chnl in channel_list:
												await self.bot.get_channel(chnl).send(f'```자동 미입력 횟수 {basicSetting[17]}회 초과! [{bossData[i][0]}] 삭제!```', tts=False)
											print ('자동미입력 횟수초과 <' + bossData[i][0] + ' 삭제완료>')
										else:
											for chnl in channel_list:
												await self.bot.get_channel(chnl).send(f'```자동 멍처리 횟수 {basicSetting[17]}회 초과! [{bossData[i][0]}] 삭제!```', tts=False)
										...
										if bossData[i][2] == '0':
											...
											for chnl in channel_list:
												await self.bot.get_channel(chnl).send("```" +  bossData[i][0] + ' 미입력 됐습니다.```', tts=False)
											embed = discord.Embed(
										...
										################ 멍 보스 ################
											...
											tmp_bossDateString[i] = bossDateString[i] = nextTime.strftime('%Y-%m-%d')
											for chnl in channel_list:
												await self.bot.get_channel(chnl).send("```" + bossData[i][0] + ' 멍 입니다.```')
											embed = discord.Embed(
												description= '```다음 ' + bossData[i][0] + ' ' + bossTimeString[i] + '입니다.```',
												color=0xff0000
												)
	...
	async def reaction_check(self, msg):
		...
		try:
			for chnl in channel_list:
				temp: discord.message.Message = await self.bot.get_channel(chnl).send(everyone+msg, tts=False)
			...
			for i in range(waiting_time):
				for message in message_list:
					try:
						await message.edit(content=everyone+msg + f"(잔여시간 : {waiting_time - i}초)")
					except:
						pass
				await asyncio.sleep(1)
			...
			usr_count = 0
			for chnl in channel_list:
				guild = self.bot.get_channel(chnl).guild.name
				if guild not in guild_list:
					result_dic[guild] = []
					guild_list.append(guild)

				cache_msg = await self.bot.get_channel(chnl).fetch_message(message_list[k].id)
				...
				for user in participant_users:
					result_dic[guild].append(user.mention)
					usr_count+=1
				k+=1
			final_msg = msg + "\n" +f"👥 참가자 ({usr_count}명)"
...
class mainCog(commands.Cog): 
	...
	async def join_(self, ctx):
		...
		if basicSetting[7] == "":
			...
			print('< 메인서버 [' + ctx.guild.name + '] 접속완료>')
	...
	async def menu_(self, ctx):
		...
		command_list += ','.join(command[43]) + ' 도움을 요청합니다\n' #!도움
		command_list += ','.join(command[44]) + ' 채널을 등록합니다\n' #!등록
	...
	async def nearTimeBoss_(self, ctx):
		...
		if len(sorted_datelist) == 0:
			...
			await ctx.send( embed=embed, tts=False, delete_after=60)
	...
	async def call_help_(self, ctx, *, args : str = None):
		global channel_list
		try:
			msg = ""
			if args:
			...
			for chnl in channel_list:
				if guild_id != self.bot.get_channel(chnl).guild.id:
					await self.bot.get_channel(chnl).send(msg+everyone, delete_after=60, tts=False)
			await ctx.send("도움 요청 완료", delete_after=60, tts=False)
		except Exception as e:
			print(str(e))

	################ 등록 ################
	@commands.command(name=command[44][0], aliases=command[44][1:])
	async def call_help_(self, ctx):
		global channel_list
		try:
			channel_id = ctx.channel.id
			if channel_id not in channel_list:
				channel_list.append(channel_id)
				await ctx.send("채널 등록 완료", delete_after=60, tts=False)
		except:
			pass
...
class IlsangDistributionBot(commands.AutoShardedBot):
	...
	async def on_ready(self):
		...
		if basicSetting[7]:
			...
			setting_channel_name = self.get_guild(basicSetting[7]).name

			now = datetime.datetime.now() + datetime.timedelta(hours = int(basicSetting[0]))

			print('< 접속시간 [' + now.strftime('%Y-%m-%d ') + now.strftime('%H:%M:%S') + '] >')
			print('< 메인서버 [' + self.get_guild(basicSetting[7]).name + '] 접속완료>')
		...
		if chflg == 1 :
			...
			for i in range(bossNum):
				...
				if message.content.startswith(bossData[i][0] +'컷') or message.content.startswith(convertToInitialLetters(bossData[i][0] +'컷')) or message.content.startswith(bossData[i][0] +' 컷') or message.content.startswith(convertToInitialLetters(bossData[i][0] +' 컷')):
					...
					print('```다음 ' + bossData[i][0] + ' ' + bossTimeString[i] + '입니다.```')
					await self.get_channel(channel).send(embed=embed, tts=False, delete_after=60)
